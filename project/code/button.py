...
class Button:
    def __init__(self, button_pin, led_pin, callback, identity, debounce_time_ms=10, on_release=False, *args, **kwargs):
    ...
    def process_button(self, timer):
        ...
        if button_current_state != self.button_last_state:
            if button_current_state and not self.on_release:  # Button pressed and callback on press
                self.led.on_ms(100)
                self._callback_wrapper()  # Use the wrapper to call the callback with additional arguments
            elif not button_current_state and self.on_release:  # Button released and callback on release
                self._callback_wrapper()  # Use the wrapper to call the callback with additional arguments
