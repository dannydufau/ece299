        ...
        if id in auxiliary_menu_map:
            ...
            if id == "stop_monitoring":
                ...
                if radio_control.muted:
                    ...
                    return start_volume_config(navigation_display, radio_control)  # Show volume config as a placeholder
    ...
    try:
        ...
        if next_ui_id:
            ...
            if next_ui_id in menu_map:
                ...
                elif next_ui_id in ["set_volume", "set_frequency", "frequency_fractional"]:
    ...
    try:
        while not thread_manager.is_stopped():
            ...
            if context_queue.size() > 0:
                ...
                logger.debug(f"runner,nav_mon,dequeue\n{context.router_context}\n{context.ui_context}\n{context_queue.size()}")
                ...
                if context:
                    ...
                    #while menu.is_encoder_button_pressed():
                    #   logger.info("Encoder button is still pressed...")
                    #   utime.sleep(0.1)
            utime.sleep(0.1)
...
def display_radio_status():
    try:
        ...
        if not radio_control.muted:
            ...
            radio_status_text = "{:.1f}MHz Vol: {}".format(current_frequency, current_volume)
...
def timer_callback(timer, auxiliary_queue):
    try:
        ...
        if current_time - boot_time < 5:
            ...
            report_display._clear_row(0)  # Clear the battery status display after 5 seconds
            report_display._clear_row(1)  # Clear the battery status display after 5 seconds
...
try:
    ...
    timer.init(period=1000, mode=Timer.PERIODIC, callback=lambda t: timer_callback(t, auxiliary_queue))
